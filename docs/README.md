...
Fast, portable image processing for Node agents. Prism runs in-process with
...
const jpeg = await prism.toJpeg(imageBuffer, { maxSide: 1600, quality: 85 });
```
...
| Page | What it covers |
| --- | --- |
| [Configuration](./configuration.md) | `createPrism`, options, pixel budgets, env vars, custom command resolution |
...
| [Error handling](./error-handling.md) | `PrismUnavailableError`, `isPrismUnavailableError` |

## Two ways to call

Create a configured instance with `createPrism(options)`, or use the
...
directly and share a single default `Prism` instance.

## Safety model

Prism refuses to process images it can't size up. It rejects images with
